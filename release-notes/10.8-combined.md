...
* The download filter policy is not triggering due to invalid URL partial match logic.
* Local groups on macOS without a SID prevents local user inventory from completing.
* Mac agent experiences database contention when Office for Mac is installed or updated.

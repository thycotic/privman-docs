...
* Server side enforcement of TLS 1.2.
...
* Endpoint Security system extension (SYSEX) replacing most functionality previously provided by the Kernel Extension (KEXT).
* The __AdjustEffectiveProcessRightsContract__ action has been deprecated for endpoint running macOS Big Sur. The __Run as Root__ action has to be used in policies instead.
...
* Big Sur System Extension Agent limitations:
  * Deprecated __AdjustEffectiveProcessRightContract__ action.
  * Deprecated Finder Extension support.
  * Policies that elevate application execution, need to support command line execution via `sudo` plugin.
